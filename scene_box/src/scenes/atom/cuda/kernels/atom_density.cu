    ...
    const int l, const int abs_m)
{
    ...
    float rho_pow = 1.0f;
    if (l > 0)
        rho_pow = powf(rho, (float)(2 * l));
    float sin_pow = 1.0f;
    if (abs_m > 0)
        sin_pow = powf(sin_sq, (float)abs_m);
    float poly_sq = (R_poly * R_poly) * (Y_poly * Y_poly);
    float exp_val = __expf(-rho);
    float density = prefactor * exp_val * rho_pow * sin_pow * poly_sq;
    size_t idx = (size_t)vz * dimY * dimX + (size_t)vy * dimX + vx;
    output_grid[idx] = density;
}
