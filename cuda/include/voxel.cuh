...
unsigned int voxel_index(unsigned int x, unsigned int y, unsigned int z,
                         const VoxelGridParams& params) {
    return z * params.dim_y * params.dim_x + y * params.dim_x + x;
}

__device__ __forceinline__
float sample_voxel(const Voxel* voxels, int x, int y, int z,
                   const VoxelGridParams& params) {
...
__device__ __forceinline__
float sample_voxel_trilinear(const Voxel* voxels, float wx, float wy, float wz,
                             const VoxelGridParams& params) {
